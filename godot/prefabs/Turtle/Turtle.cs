...
public partial class Turtle : CharacterBody3D
{
	...
	private const float c_maxSpeed = 8; // calculated by looking at the debug outputs :) --- this should be calculated
	...
	AnimationTree m_animation;
	Label? m_debugText;

	public override void _Ready()
	{
		Transport = GetNode<Transport>("model/transport");
		m_animation = (AnimationTree)FindChild("AnimationTree");
	...
	void HandleMovement(double deltaTimeD)
	{
		...
		if (TestMove(GlobalTransform, motion, m_collision))
		{
			...
			if (relativeOrientation > -0.6f)
			{
				...
				m_forwardSpeed *= 1 - (0.1f * deltaTime);
			}
			else
			{
				motion = Vector3.Zero;
				m_forwardSpeed *= 1 - (0.9f * deltaTime);
			}
	...
	public override void _Process(double deltaTime)
	{
		...
		m_animation.Set("parameters/BlendSpace1D/blend_position", m_forwardSpeed);
		m_animation.Set("parameters/TimeScale/scale", Mathf.Max(1, m_forwardSpeed / 2));
	}
}
